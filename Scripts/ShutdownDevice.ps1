...
Invoke-expression -Command:("C:\Program Files\Citrix\ICA Client\SelfServicePlugin\selfservice.exe -init -ipoll")

#Launch the published desktop
If ($autoRunApp) {
    Invoke-expression -Command:("C:\Program Files\Citrix\ICA Client\SelfServicePlugin\SelfService.exe -qlaunch $applicationName")
}
...
If ($removeUserProfile) {
    ...
    #Write out a registry file to import the relevant settings
